...
def agregar_mascota(request):
    ...
    if request.method == 'POST':
        ...
        mascota = Mascota(nombre=nombre, especie=especie, raza=raza, sexo=sexo, edad=edad, id_dueño=dueño, peso=peso)
        mascota.save()
...
def realizar_actualizacion_mascota(request, id_mascota):
    ...
    if request.method == 'POST':
        ...
        hoy = datetime.now()
        fecha_nac = datetime.strptime(fecha_nacimiento, '%Y-%m-%d')
        mascota.edad = hoy.year - fecha_nac.year - ((hoy.month, hoy.day) < (fecha_nac.month, fecha_nac.day))
